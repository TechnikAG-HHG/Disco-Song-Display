...
def search_user(query):
    ...
    search_url = "https://api.spotify.com/v1/search"
    headers = {"Authorization": f"Bearer {token}"}
    params = {"q": query, "type": "track", "market": "DE", "limit": 50}
    try:
        print(query)
        response = requests.get(search_url, headers=headers, params=params).json()
        #print(response)
...
def add_to_blacklist(track_name):
    ...
    if len(selection) > 0:
        ...
        #parts = track.rsplit(" - ", 1)  # Trennen des Strings anhand des Trennzeichens und des letzten Elements
          # Entfernen des letzten Elements und Zusammenfügen des Strings
...
def blacklist_button_click():
    selection = search_results.curselection()
    if len(selection) > 0:
        tracka = search_results.get(selection[0])


        #print(tracka)
        artist_name, track_name = tracka.split(" - ",1)
        add_to_blacklist(f"{replace_non_ascii(clean_name_from_breakin_things(artist_name))} - {replace_non_ascii(clean_name_from_breakin_things(track_name))}")
        search_results.delete(selection[0])
        #tkinter.messagebox.showinfo("Success", f"Added '{replace_non_ascii(clean_name_from_breakin_things(artist_name)) } - {replace_non_ascii(clean_name_from_breakin_things(track_name))}' to the blacklist.")
    else:
...
root = tk.Tk()
root.title("Spotify SR Blacklister")
...
# Create search bar
coollabel = tk.Label(text="Blacklister", font=('Helvetica', 48, 'bold'), bg='#0047AB', fg='white')
coollabel.pack(side=tk.TOP, pady=20)
search_frame = tk.Frame(root, bg='#0047AB')
search_frame.pack(side=tk.TOP, padx=padding, pady=padding)
search_label = tk.Label(search_frame, text="Suche:", font=('Arial', 32), bg='#0047AB', fg='white')
...
add_button = tk.Button(root, text="Blacklist", font=('Arial', 30), command=blacklist_button_click)
